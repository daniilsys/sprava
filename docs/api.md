...
| Method | Path | Purpose | Auth | Request | Success response | Error response |
| --- | --- | --- | --- | --- | --- | --- |
| GET | `/media` | Retrieve informations about a media by its id| No (header accepted but not validated) | Query: `media_id` | JSON: `{ id, filename, file_size, message_id }` | `404` HTTP error |
